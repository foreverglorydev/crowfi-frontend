...
const formatCrowFactoryResponse = (rawCrowFactory?: CrowFactory) => {
  if (rawCrowFactory) {
    return {
      totalTransactions: parseFloat(rawCrowFactory.totalTransactions),
      totalVolumeUSD: parseFloat(rawCrowFactory.totalVolumeUSD),
      totalLiquidityUSD: parseFloat(rawCrowFactory.totalLiquidityUSD),
    }
  }
  return null
}
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const overviewData = formatCrowFactoryResponse(data?.crowFactories?.[0])
      const overviewData24 = formatCrowFactoryResponse(data24?.crowFactories?.[0])
      const overviewData48 = formatCrowFactoryResponse(data48?.crowFactories?.[0])
